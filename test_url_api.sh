...
# -----------------------------------------------------------------------------
# Test: should normalize URLs when encoding
# Corresponds to: test "should normalize URLs when encoding"
# -----------------------------------------------------------------------------
print_test "should normalize URLs when encoding"
echo "Testing URL normalization with different formats of the same URL"
...
echo "First request: POST ${BASE_URL}/encode with { \"url\": \"${URL1}\" }"
RESPONSE1=$(curl -s -X POST "${BASE_URL}/encode" \
  -H "Content-Type: application/json" \
  -d "{\"url\": \"${URL1}\"}")
...
echo "Second request: POST ${BASE_URL}/encode with { \"url\": \"${URL2}\" }"
RESPONSE2=$(curl -s -X POST "${BASE_URL}/encode" \
  -H "Content-Type: application/json" \
  -d "{\"url\": \"${URL2}\"}")
...
if [ "$RESPONSE1" = "$RESPONSE2" ]; then
  echo -e "${GREEN}✓ Same shortened URL for normalized URLs: PASS${NC}"
else
  echo -e "${RED}✗ Different shortened URLs for normalized URLs: FAIL${NC}"
fi
